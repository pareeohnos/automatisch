...
export default defineAction({
  ...
  arguments: [
    ...
    },
    {
      ...
      required: true,
      description: 'What is the practitest ID of the issue',
      variables: true,
    },
    {
      label: 'Name',
  ...
  async run($) {
    ...
    const practitestId = $.step.parameters.id;
    ...
    const response = await $.http.put(
      `/projects/${project}/issues/${practitestId}.json`,
      {
        data: {
          type: 'issues',
          attributes,
    ...
    const statusResponse = await $.http.put(
      `/projects/${project}/issues/${practitestId}/update_status.json`,
      {
        data: {
          type: 'issues',
          attributes: {
            'status-name': $.step.parameters.status,
          },
        },
      }
    );
